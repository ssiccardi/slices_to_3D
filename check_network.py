...
nodes = list(csv.reader(open('data/nodes.csv'), delimiter=","))
...
writer.writerow(['id', 'z', 'x', 'y', 'nodes linked'])

for node in mynodes:
    if node[5] == True:
        writer.writerow([node[0],node[1],node[2],node[3],node[4]])
        nnodes = nnodes+1

#out=base64.encodestring(buf.getvalue())
text_file = open("data/nodes_ok.csv", "w")
